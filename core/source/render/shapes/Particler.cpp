...
fun::Particler::Particler(RenderType rt, uint32_t mc, uint8_t tc) : render_type(rt), max_count(mc), thread_count(tc) {
    ...
    if (thread_count > 1) {
        ...
        for (int i = 0; i < thread_count; i++) {
            thread_pool[i] = std::thread(CallData(i, this));
        }
    }

    if (render_type == RenderType::Quads) {
        particles = new ParticleData[max_count];
        vertices = new sf::Vertex[max_count * 4];
    }
}

fun::Particler::~Particler() {
    // ? destroy threads
...
fun::Particler::CallData::CallData(int id, Particler* particler) : id(id), particler(particler) {}

void fun::Particler::CallData::operator ()() {
    ThreadData& data = particler->thread_data[id];
...
void fun::Particler::Emit(uint32_t emit_count, const EmitData& data) {
