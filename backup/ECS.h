...
namespace fun::ecs {
    ...
    std::vector <uint8_t>* get_component_array(ComponentID component_id);
    std::vector <Entity>*  get_entities_array(ComponentID component_id);

    template <class T>
    ComponentIterator <T> iterate_component() {
        return ComponentIterator <T> (get_component_array(get_component_id <T> ()));
    }
    ...
    template <class T>
    void add_component(Entity entity, std::function <void(uint8_t*)> create_component = [](uint8_t* ptr) -> void { *reinterpret_cast <T*> (ptr) = T(); }) {
        add_component(entity, get_component_id <T> (), sizeof(T), create_component);
    }
